...
class FsReqBranches():
    ...
    def show_channels_as_json(self):
    ...
    def sofia_xmlstatus_profile_internal_reg(self): ## app/registrations/status_registrations.php
        self.logger.debug('branch "status_registrations"')
        root = xmlET.Element('profile')
        t = xmlET.Element('registrations')
        for x in vars.rfs:
            if x['resp'][0:6] == "<?xml ":
                root.extend(xmlET.fromstring(x['resp']))
        t.extend(root.find("registrations"))    ## take only one registration (from one FS-node) because of shared memcache in my config
        for registration in t.iter("registration"):
    ...
    def sofia_xmlstatus(self):
        self.logger.debug('branch "sip status"')
        root = xmlET.Element('profiles')
        t = xmlET.fromstring("<profile><name /><type /><data /><state>NOT RUNNING (refresh page in 30 sec)</state></profile>")
        for num,x in enumerate(vars.rfs):
            cur = xmlET.fromstring(x['resp'])
            if x['resp'][0:6] == "<?xml " and len(cur): #'and' to exclude "<profiles />" variant of xml-responce
                root.extend(cur)
            else:
    ...
    def sofia_xmlstatus_profile_internal(self):
        ...
        for num, x in enumerate(vars.rfs):
            if x['resp'][0:6] == "<?xml ":
                ...
                for sub in sub_elements:
                    t.find(sub).text += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;FS" + str(num) + ": " + root.findtext(sub, default =" ")
            else:
                t.find("name").text += "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;FS" + str(num) + ": " + x['resp']
        if len(root):
            for sub in sub_elements:
                root.find('profile-info').find(sub).text = t.findtext(sub)
        else:
            root.append(t)
        body_xml = xmlET.tostring(root, encoding="ISO-8859-1", method="xml") + "\n"
